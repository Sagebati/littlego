...
class PrioritizedSumTree:
	...
	def save_sumTree(self):
		np.savez(sumTreeFile, 
			tree = self.tree, 
			pointer_idx = self.pointer_idx)
	
	def restore_sumTree(self):
		npzFile = np.load("{}.npz".format(sumTreeFile))
		self.tree = npzFile['tree']
		self.pointer_idx = npzFile['pointer_idx']
